...
import { GitBranch, AlertCircle, Eye, EyeOff, Loader2 } from 'lucide-react'
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-black text-white">
            Vyn<span className="text-emerald-400">CICD</span>
          </h1>
          <p className="text-sm text-slate-500 mt-1">AI-Powered CI/CD Platform</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-[#111827] border border-slate-800 rounded-2xl p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="admin@vyncicd.local"
          ...
          <button type="submit" disabled={loading}
            className="w-full py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-white font-bold text-sm transition-colors flex items-center justify-center gap-2">
        ...
        <div className="mt-6 rounded-xl border border-slate-800 bg-slate-900/40 px-4 py-3 text-xs text-slate-500 space-y-1.5">
          ...
          <p>Set <span className="font-mono text-slate-400">VYNCICD_ADMIN_EMAIL</span> and <span className="font-mono text-slate-400">VYNCICD_ADMIN_PASSWORD</span> in <span className="font-mono text-slate-400">.env.local</span> — the admin account is created automatically on first boot.</p>
        </div>

        <p className="text-center text-xs text-slate-600 mt-4">
          Part of the{' '}
          <a href="https://vynops.com/" target="_blank" rel="noopener noreferrer" className="text-emerald-500 hover:text-emerald-400 transition-colors">VynOps Suite</a>
        </p>
      </div>
    </div>
  )
